...
### 1. 이진 조기경보 (헤드라인) — 시간순 CV, 베이스라인 대비
...
![PR 커브 — 모델 vs 베이스라인](reports/figures/모델_pr_curve.png)

### 2. 다중분류 — 경보 단계(정상/관심/경계이상)

누적 이진(임계 1,000·10,000) 방식. 정확도는 정상 클래스(89.8%) 때문에 오해를 부르므로 **상위 단계 recall**로 평가 — 위험한 **경계이상 recall 0.786** (persistence 0.602 대비 우위).

![경보 단계 혼동행렬](reports/figures/모델_혼동행렬.png)

### 3. GBDT vs 딥러닝 — 동일 피처·분할·지표 공정 비교

테이블형·소규모(3.4만행) 특성상 **LightGBM 우위**, 논문에 충실한 FT-Transformer도 근소차로 추격하나 못 넘고, MLP는 열위. "이 데이터에선 GBDT가 맞다"는 근거를 정직하게 확보(딥러닝이 이기지 못한 음성 결과 포함). *(pooled OOF 기준 — 헤드라인 0.807은 폴드 평균이라 집계 방식이 다름)*

![GBDT vs 딥러닝 벤치마크](reports/figures/모델_dl_benchmark.png)

### 4. 해석 (SHAP)

현재 유해남조류 세포수·수온·클로로필-a와 그 과거값(lag/rolling)이 주요 기여 피처. 예측 근거를 확인 가능해 신뢰성·의사결정 지원에 유리.

![SHAP 피처 기여도](reports/figures/shap_bar.png)
...
MIT (`LICENSE`). 서드파티 고지 `THIRD_PARTY_NOTICES`, 의존성 스캔 `reports/license_report.md`.
