...
```bash
# 1) 데이터 취득 (조류 전 지점·전월)
uv run python scripts/fetch_algae.py --start-year 2015 --end-year 2025
...
```bash
uv run pytest      # 로더·타깃·피처·검증 (누수 회귀 테스트 포함)
```
...
- 다중분류(경보 단계: 관심/경계/대발생)
- 글로벌 딥러닝 벤치마크(TFT·N-HiTS 등) vs GBDT
- 실시간 기상 **예보** 연동(예측 대상 미래 구간) + 대시보드
